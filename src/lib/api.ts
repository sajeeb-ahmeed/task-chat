...
export async function request<T>(path: string, token?: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) {
    ...
    if (token && (response.status === 401 || code === 'NO_TOKEN'))
      window.dispatchEvent(new Event('thread:session-expired'));
    throw new ApiError(
      response.status >= 500
        ? 'The chat server is having trouble. Please try again shortly.'
...
export const api = {
  ...
  group: (token: string, name: string, participantIds: string[]) =>
    request<Conversation>('/conversations/group', token, post({ name, participantIds })),
  history: (token: string, id: string, before?: string, signal?: AbortSignal) =>
    request<MessagePage>(
      `/conversations/${encodeURIComponent(id)}/messages?limit=30${before ? `&before=${encodeURIComponent(before)}` : ''}`,
      token,
      { signal },
    ),
  send: (token: string, conversationId: string, text: string) =>
    request<Message>('/messages', token, post({ conversationId, text: text.trim() })),
};
