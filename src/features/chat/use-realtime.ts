...
export function useRealtime(session: Session) {
  ...
  useEffect(() => {
    ...
    const reconcile = () => { void client.invalidateQueries({ queryKey: ['conversations'] }); void client.invalidateQueries({ queryKey: ['messages'] }); };
    ...
    socket.on('message:new', (payload: unknown) => {
      ...
      if (!message) { reconcile(); return; }
      client.setQueryData<Message[]>(['live', session.user._id, message.conversation], (old = []) => [...old.filter(m => m._id !== message._id), message]);
