...
## Portfolio protocol and intermittent browser timeout follow-up

The user's screenshot exposed `PARA:` prefixes that Markdown alone cannot interpret. Inspection of the deployed portfolio's `sajib-ai.js` confirmed its separate `SECTION:`, `PARA:`, `ITEM:`, `CODE_START: language` / `CODE_END` protocol. The first formatting fix missed this contract. A display adapter now converts these markers into headings, paragraphs, lists and fenced code, preserves protocol-looking text inside code, and uses the same formatted text for copying. Ordinary Markdown remains supported.

The reported failures affected `/`, `/login?_rsc=...` and the AI relay together, and included Chrome's full-page `ERR_CONNECTION_TIMED_OUT`. The normal Chrome tab was observed in that error state; reloading recovered it and login navigation then succeeded. Read-only HTTP checks of landing/login returned 200, including explicit connections to both resolved IPv4 addresses (64.29.17.131 and 216.198.79.131). The user reports Incognito works while normal browsing sometimes fails. This narrows investigation toward intermittent browser-profile/network reachability but does not identify a specific extension, cache or ISP cause. No system DNS, browser security setting, extension or firewall was changed. No permanent network fix is claimed.

An AI transport failure now explains that the site could not be reached, preserves the draft for a manual retry, and offers the independent portfolio destination. It does not automatically replay POST requests. A site that cannot load at all cannot repair its connection using React code. Separate tests cover the screenshot's Bengali protocol case, literal code markers and an aborted network request with retained draft. Follow-up checks: 17 unit tests, 5 focused AI browser tests, lint and build passed.
